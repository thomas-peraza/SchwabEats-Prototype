    ...
    <div class="px-4 py-8 grid lg:grid-cols-[2fr_1fr] gap-8">
      ...
      <section>
        ...
        <div class="grid gap-6">
          <router-link to="/vendor-orders"
            class="bg-white rounded-2xl shadow-xl p-8 hover:shadow-2xl active:scale-[0.98] transition-transform duration-150">
          ...
          <router-link to="/vendor-menu"
            ...
            <p class="text-5xl font-bold mt-6">9</p>
          </router-link>
      ...
      <aside class="space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-xl p-6">
          ...
          <svg viewBox="0 0 400 220" class="w-full h-64 bg-gray-50 rounded-xl p-4">
            <polyline v-if="linePoints" :points="linePoints" fill="none" stroke="#1FABFF" stroke-width="5"
              stroke-linecap="round" stroke-linejoin="round" />

            <circle v-for="point in graphPoints" :key="point.x + '-' + point.y" :cx="point.x" :cy="point.y" r="5"
              fill="#455A64" />

            <text v-if="graphPoints.length === 0" x="200" y="110" text-anchor="middle" fill="#64748b" font-size="18">
              No revenue data yet
            </text>
          </svg>
